...
CONF_DISTANCE_UPDATE_INTERVAL = "distance_update_interval"
...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_DISTANCE_UPDATE_INTERVAL, default="1s"): cv.update_interval,
...
async def to_code(config):
    ...
    if CONF_DISTANCE_UPDATE_INTERVAL in config:
        update_interval = config[CONF_DISTANCE_UPDATE_INTERVAL]
        cg.add(var.set_distance_update_interval(update_interval))